...
- title: the Pinterest SEARCH surface, not the hook. Every title in the batch MUST carry the core search terms -- the brand and what the product actually is -- because each pin is a separate chance to rank for the same query, so repeating those terms across pins is correct and wanted. But there is NO required slot order: do not fill a fixed "brand + size + colour" template, because filling the same template ten times produces near-identical titles. Instead let each pin FOREGROUND a different true detail of this product -- a size, a material, a feature, a room or place it suits, a problem it solves -- and build the title around that detail while still carrying the core terms. Never produce a title identical to another pin in the batch. Do NOT phrase the title as a question; a hook belongs in overlay_headline. Numbers are good when true. Max 100 chars. Must NOT start with a BANNED OPENING WORD.
